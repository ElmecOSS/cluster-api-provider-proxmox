...
func reconcileBootstrapData(ctx context.Context, machineScope *scope.MachineScope) (requeue bool, err error) {
	...
	if err != nil {
		conditions.MarkFalse(machineScope.ProxmoxMachine, infrav1alpha2.VMProvisionedCondition, infrav1alpha2.WaitingForStaticIPAllocationReason, clusterv1.ConditionSeverityWarning, "%s", err)
		return false, err
	}
...
func injectCloudInit(ctx context.Context, machineScope *scope.MachineScope, bootstrapData []byte, biosUUID string, nicData []types.NetworkConfigData, kubernetesVersion string) error {
	...
	if err := injector.Inject(ctx, inject.CloudConfigFormat); err != nil {
		conditions.MarkFalse(machineScope.ProxmoxMachine, infrav1alpha2.VMProvisionedCondition, infrav1alpha2.VMProvisionFailedReason, clusterv1.ConditionSeverityWarning, "%s", err)
		return err
	}
	return nil
}
...
func getCommonInterfaceConfig(ctx context.Context, machineScope *scope.MachineScope, ciconfig *types.NetworkConfigData, nic infrav1alpha2.AdditionalNetworkDevice) error {
	...
	if ippool := nic.NetworkDevice.IPv4PoolRef; ippool != nil && ciconfig.IPAddress == "" {
		// retrieve IPAddress.
		var ifname = fmt.Sprintf("%s-%s", ciconfig.Name, infrav1alpha2.DefaultSuffix)
	...
	if nic.NetworkDevice.IPv6PoolRef != nil && ciconfig.IPV6Address == "" {
		var ifname = fmt.Sprintf("%s-%s", ciconfig.Name, infrav1alpha2.DefaultSuffix+"6")
		ipAddr, err := findIPAddress(ctx, machineScope, ifname)
		if err != nil {
			return errors.Wrapf(err, "unable to find IPAddress, device=%s", ifname)
		}
...
func getVirtualNetworkDevices(_ context.Context, _ *scope.MachineScope, network infrav1alpha2.NetworkSpec, data []types.NetworkConfigData) ([]types.NetworkConfigData, error) {
...
func getAdditionalNetworkDevices(ctx context.Context, machineScope *scope.MachineScope, network infrav1alpha2.NetworkSpec) ([]types.NetworkConfigData, error) {
	...
	for _, nic := range network.AdditionalDevices {
		...
		if nic.IPv4PoolRef != nil {
			device := fmt.Sprintf("%s-%s", nic.Name, infrav1alpha2.DefaultSuffix)
		...
		if nic.IPv6PoolRef != nil {
			suffix := infrav1alpha2.DefaultSuffix + "6"
