...
func (r *ProxmoxMachineReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&infrav2alpha2.ProxmoxMachine{}).
		Watches(
			&clusterv1.Machine{},
			handler.EnqueueRequestsFromMapFunc(util.MachineToInfrastructureMapFunc(infrav2alpha2.GroupVersion.WithKind(infrav2alpha2.ProxmoxMachineKind))),
		).
		Complete(r)
}
...
func (r *ProxmoxMachineReconciler) reconcileDelete(ctx context.Context, machineScope *scope.MachineScope) (ctrl.Result, error) {
	...
	conditions.MarkFalse(machineScope.ProxmoxMachine, infrav2alpha2.VMProvisionedCondition, clusterv1.DeletingReason, clusterv1.ConditionSeverityInfo, "")
	...
	// VM is being deleted
	return reconcile.Result{RequeueAfter: infrav2alpha2.DefaultReconcilerRequeue}, nil
}
...
func (r *ProxmoxMachineReconciler) reconcileNormal(ctx context.Context, machineScope *scope.MachineScope, clusterScope *scope.ClusterScope) (reconcile.Result, error) {
	...
	if !machineScope.Cluster.Status.InfrastructureReady {
		...
		conditions.MarkFalse(machineScope.ProxmoxMachine, infrav2alpha2.VMProvisionedCondition, infrav2alpha2.WaitingForClusterInfrastructureReason, clusterv1.ConditionSeverityInfo, "")
		return ctrl.Result{}, nil
	}
	...
	if machineScope.Machine.Spec.Bootstrap.DataSecretName == nil {
		...
		conditions.MarkFalse(machineScope.ProxmoxMachine, infrav2alpha2.VMProvisionedCondition, infrav2alpha2.WaitingForBootstrapDataReason, clusterv1.ConditionSeverityInfo, "")
		return ctrl.Result{}, nil
	}

	// If the ProxmoxMachine doesn't have our finalizer, add it.
	if ctrlutil.AddFinalizer(machineScope.ProxmoxMachine, infrav2alpha2.MachineFinalizer) {
	...
	if vm.State != infrav2alpha2.VirtualMachineStateReady {
		machineScope.Logger.Info(
			"VM state is not reconciled",
			"expectedVMState", infrav2alpha2.VirtualMachineStateReady,
	...
	conditions.MarkTrue(machineScope.ProxmoxMachine, infrav2alpha2.VMProvisionedCondition)
	machineScope.Logger.Info("ProxmoxMachine is ready")

	return reconcile.Result{}, nil
}

func (r *ProxmoxMachineReconciler) getInfraCluster(ctx context.Context, logger *logr.Logger, cluster *clusterv1.Cluster, proxmoxMachine *infrav2alpha2.ProxmoxMachine) (*scope.ClusterScope, error) {
