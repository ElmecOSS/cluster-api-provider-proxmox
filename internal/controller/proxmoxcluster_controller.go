...
func (r *ProxmoxClusterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&infrav2alpha2.ProxmoxCluster{}).
		WithEventFilter(predicates.ResourceNotPaused(r.Scheme, ctrl.LoggerFrom(ctx))).
		Watches(&clusterv1.Cluster{},
			handler.EnqueueRequestsFromMapFunc(clusterutil.ClusterToInfrastructureMapFunc(ctx, infrav2alpha2.GroupVersion.WithKind(infrav2alpha2.ProxmoxClusterKind), mgr.GetClient(), &infrav2alpha2.ProxmoxCluster{})),
...
func (r *ProxmoxClusterReconciler) reconcileDelete(ctx context.Context, clusterScope *scope.ClusterScope) (reconcile.Result, error) {
	...
	if len(machines) > 0 {
		...
		return ctrl.Result{RequeueAfter: infrav2alpha2.DefaultReconcilerRequeue}, nil
	}
	...
	ctrlutil.RemoveFinalizer(clusterScope.ProxmoxCluster, infrav2alpha2.ClusterFinalizer)
	return ctrl.Result{}, nil
}
...
func (r *ProxmoxClusterReconciler) reconcileNormal(ctx context.Context, clusterScope *scope.ClusterScope) (reconcile.Result, error) {
	...
	ctrlutil.AddFinalizer(clusterScope.ProxmoxCluster, infrav2alpha2.ClusterFinalizer)
	...
	if clusterScope.ProxmoxCluster.Spec.ExternalManagedControlPlane {
		if clusterScope.ProxmoxCluster.Spec.ControlPlaneEndpoint == nil {
			...
			conditions.MarkFalse(clusterScope.ProxmoxCluster, infrav2alpha2.ProxmoxClusterReady, infrav2alpha2.MissingControlPlaneEndpointReason, clusterv1.ConditionSeverityWarning, "The ProxmoxCluster is missing or waiting for a ControlPlaneEndpoint")
		...
		if clusterScope.ProxmoxCluster.Spec.ControlPlaneEndpoint.Host == "" {
			...
			conditions.MarkFalse(clusterScope.ProxmoxCluster, infrav2alpha2.ProxmoxClusterReady, infrav2alpha2.MissingControlPlaneEndpointReason, clusterv1.ConditionSeverityWarning, "The ProxmoxCluster is missing or waiting for a ControlPlaneEndpoint host")
	...
	if err := r.reconcileNormalCredentialsSecret(ctx, clusterScope); err != nil {
		conditions.MarkFalse(clusterScope.ProxmoxCluster, infrav2alpha2.ProxmoxClusterReady, infrav2alpha2.ProxmoxUnreachableReason, clusterv1.ConditionSeverityError, "%s", err)
		if apierrors.IsNotFound(err) {
	...
	conditions.MarkTrue(clusterScope.ProxmoxCluster, infrav2alpha2.ProxmoxClusterReady)
...
func (r *ProxmoxClusterReconciler) reconcileIPAM(ctx context.Context, clusterScope *scope.ClusterScope) (reconcile.Result, error) {
	...
	if clusterScope.ProxmoxCluster.Spec.IPv4Config != nil {
		poolV4, err := clusterScope.IPAMHelper.GetDefaultInClusterIPPool(ctx, infrav2alpha2.IPV4Format)
		if err != nil {
			if apierrors.IsNotFound(err) {
				return ctrl.Result{RequeueAfter: infrav2alpha2.DefaultReconcilerRequeue}, nil
			}
	...
	if clusterScope.ProxmoxCluster.Spec.IPv6Config != nil {
		poolV6, err := clusterScope.IPAMHelper.GetDefaultInClusterIPPool(ctx, infrav2alpha2.IPV6Format)
		if err != nil {
			if apierrors.IsNotFound(err) {
				return ctrl.Result{RequeueAfter: infrav2alpha2.DefaultReconcilerRequeue}, nil
			}
...
func (r *ProxmoxClusterReconciler) reconcileNormalCredentialsSecret(ctx context.Context, clusterScope *scope.ClusterScope) error {
	...
	secret.SetOwnerReferences(clusterutil.EnsureOwnerRef(secret.GetOwnerReferences(),
		metav1.OwnerReference{
			APIVersion: infrav2alpha2.GroupVersion.String(),
			Kind:       "ProxmoxCluster",
	...
	if !ctrlutil.ContainsFinalizer(secret, infrav2alpha2.SecretFinalizer) {
		ctrlutil.AddFinalizer(secret, infrav2alpha2.SecretFinalizer)
	}
...
func (r *ProxmoxClusterReconciler) reconcileDeleteCredentialsSecret(ctx context.Context, clusterScope *scope.ClusterScope) error {
	...
	} else if clusterutil.HasOwnerRef(secret.GetOwnerReferences(), ownerRef) && ctrlutil.ContainsFinalizer(secret, infrav2alpha2.SecretFinalizer) {
		// There is only one OwnerRef, the current ProxmoxCluster. Remove the Finalizer (if present).
		logger.Info(fmt.Sprintf("Removing finalizer %s", infrav2alpha2.SecretFinalizer), "Secret", klog.KObj(secret))
		ctrlutil.RemoveFinalizer(secret, infrav2alpha2.SecretFinalizer)
	}

	return helper.Patch(ctx, secret)
}

func hasCredentialsRef(proxmoxCluster *infrav2alpha2.ProxmoxCluster) bool {
	return proxmoxCluster != nil && proxmoxCluster.Spec.CredentialsRef != nil
}

func getNamespaceFromProxmoxCluster(proxmoxCluster *infrav2alpha2.ProxmoxCluster) string {
